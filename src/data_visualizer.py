...
class DataVisualizer:
    ...
    def visualizeA3(self, output_path_county, output_path_sex, output_path_age, vaccinated_path):
        ...
        counties = {"Hlavní město Praha": 0, "Středočeský kraj": 0, "Jihočeský kraj": 0, "Plzeňský kraj": 0,
                    "Karlovarský kraj": 0, "Ústecký kraj": 0, "Liberecký kraj": 0, "Královéhradecký kraj": 0,
                    "Pardubický kraj": 0, "Kraj Vysočina": 0, "Jihomoravský kraj": 0, "Olomoucký kraj": 0,
                    "Zlínský kraj": 0, "Moravskoslezský kraj": 0}

        stats = {"cnt": counties.copy(), "men": counties.copy(), "women": counties.copy(), "age1": counties.copy(),
                 "age2": counties.copy(), "age3": counties.copy()}
        ...
        for county_name in counties.keys():
            ...
            # all vaccinated men
            stats["men"][county_name] = len(
                vaccinated[(vaccinated.kraj_nazev == county_name) & (vaccinated.pohlavi == "M")])

            # all women
            stats["women"][county_name] = stats["cnt"][county_name] - stats["men"][county_name]

            # all age categories
            stats["age1"][county_name] = len(
                vaccinated[(vaccinated.kraj_nazev == county_name) & (vaccinated.vekova_skupina == 1)])
            stats["age2"][county_name] = len(
                vaccinated[(vaccinated.kraj_nazev == county_name) & (vaccinated.vekova_skupina == 2)])
            stats["age3"][county_name] = stats["cnt"][county_name] - (
                    stats["age1"][county_name] + stats["age2"][county_name])
        ...
        handles = [plt.Rectangle((0, 0), 1, 1, color='#104E8B'), plt.Rectangle((0, 0), 1, 1, color='#009ACD'),
